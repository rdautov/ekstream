...
public class CaptureVideo extends EkstreamProcessor {
	...
	/** Processor property. */
	public static final PropertyDescriptor FRAME_WIDTH = new PropertyDescriptor.Builder().name("Image width")
			.description("Specifies the width of of captured frames").defaultValue("640").required(true)
			.addValidator(StandardValidators.INTEGER_VALIDATOR).build();

	/** Processor property. */
	public static final PropertyDescriptor FRAME_HEIGHT = new PropertyDescriptor.Builder().name("Image height")
			.description("Specifies the height of captured frames").defaultValue("480").required(true)
			.addValidator(StandardValidators.INTEGER_VALIDATOR).build();
	...
	public void onTrigger(final ProcessContext aContext, final ProcessSession aSession) throws ProcessException {
		...
		try {
			...
			// benchmarking=====================================
			flowFile = aSession.putAttribute(flowFile, "capture", String.valueOf(System.currentTimeMillis()));
